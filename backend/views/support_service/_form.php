...
<div class="support-service-form">
    ...
    if($chk =='create')
    {
        $type_id = $modelsubtype->type_id;
        $subtype_id = $modelsubtype->subtype_id;
        
        $type = app\models\support_service_type::find()->select(['type_name', 'type_id'])
                ->where(['type_id'=>$type_id])
                ->indexBy('type_id')->column();
        
        
        $subtype = app\models\support_service_subtype::find()->select(['subtype_name', 'subtype_id'])
                ->where(['subtype_id'=>$subtype_id])
                ->indexBy('subtype_id')->column();
    }
...
//    $serivceurl = app\models\support_Url::find()->select(['url_name', 'url_id'])->indexBy('url_id')->column();
    ...
    <?php if (!Yii::$app->user->isGuest) { $username1 = Yii::$app->user->identity->username; } ?>
        ...
        echo $form->field($model, 'urgency_id')->widget(Select2::classname(), [
            'data' => $urgency,
            'options' => ['placeholder' => 'ระดับความเร่งด่วน ...','value' => 4],
            'pluginOptions' => [
                'allowClear' => true
            ],
        ]);
